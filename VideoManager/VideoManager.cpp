...
bool AN::VideoManager::read(cv::Mat &image)
{
    ...
    else
    {
        ...
        if (elapsed > 5000)
        {
            std::cout << __YELLOW__ << __BOLD__ << "[ " << __FUNCTION__ << " | " << __LINE__ << " ]: " << __ENDC__ << __YELLOW__ << " Trying to connect" << __ENDC__ << std::endl;
            this->start_video_capture(current_vector_capture_index);
...
void AN::VideoManager::start_video_capture(int index)
{
    ...
    if (current_vector_capture_index < 0 || current_vector_capture_index >= vector_capture.size())
        std::cerr << __RED__ << __BOLD__ << "[ " << __FUNCTION__ << " | " << __LINE__ << " ]: " << __ENDC__ << __RED__ << "Index out of range" << __ENDC__ << std::endl;
    else
        std::cout << __GREEN__ << __BOLD__ << "[ " << __FUNCTION__ << " | " << __LINE__ << " ]: " << __ENDC__ << __GREEN__ << "OK" << __ENDC__ << std::endl;
